...
const char *FunctionNode::getArgumentCString() const {
    ...
    FunctionNode::getPrototypeCString();
    std::size_t start = prototype_string.find("(");
    std::size_t end = prototype_string.find(")");

    argument_string = prototype_string.substr(start + 1, end - start - 1).c_str();
    return argument_string.c_str();
}
